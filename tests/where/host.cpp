...
int main(int argc, const char *argv[])
{
	...
	xt::xarray<double> arg0 = xt::load_npy<double>("where_arg0.npy");
	xt::xarray<double> arg1 = xt::load_npy<double>("where_arg1.npy");
	xt::xarray<double> arg2 = xt::load_npy<double>("where_arg2.npy");
	xt::xarray<double> res = xt::load_npy<double>("./where_result.npy");
	xt::xarray<double> res_compute = xt::zeros_like(res);

	std::vector<double *> inputs, outputs;

	inputs = {arg0.data(), arg1.data(), arg2.data()}; 
	outputs = {res_compute.data()};
	run_where_kernel("where4d", inputs, outputs, 
			{X, Y, Z,}, {X, Y, Z,}, {X, Y, Z,}, {X, Y, Z,},                                       //shapes
