extern "C" void mult1d(double* A, double* B, int* A_stride, int* B_stride, int* A_offset, int* B_offset, double* out, int* out_shape, int* out_stride, int* out_offset, int* out_end_offset) {
...
#pragma HLS INTERFACE s_axilite port = return bundle = control
	...
	for (int i=(0 + out_offset[0]); i<(out_shape[0] + out_end_offset[0]); i++){
		out[i*out_stride[0]] = A[(i + A_offset[0])*A_stride[0]] * B[(i + B_offset[0])*B_stride[0]];
	}
}