#include <iostream>
extern "C" void where3d(double* A, double* B, double* C, int* A_stride, int* B_stride, int* C_stride, int* A_offset, int* B_offset, int* C_offset, double* out, int* out_shape, int* out_stride, int* out_offset, int* out_end_offset) {
	...
	for (int i=(0 + out_offset[0]); i<(out_shape[0] + out_end_offset[0]); i++){
		for (int j=(0 + out_offset[1]); j<(out_shape[1] + out_end_offset[1]); j++){
			for (int k=(0 + out_offset[2]); k<(out_shape[2] + out_end_offset[2]); k++){
				...
				C_ind = (i + C_offset[0])*C_stride[0] + (j + C_offset[1])*C_stride[1] + (k + C_offset[2])*C_stride[2];
