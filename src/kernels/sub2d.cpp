extern "C" void sub2d(double* A, double* B, int* A_stride, int* B_stride, int* A_offset, int* B_offset, int A_lin_offset, int B_lin_offset, double* out, int* out_shape, int* out_stride, int* out_offset, int* out_end_offset, int out_lin_offset) {
...
#pragma HLS INTERFACE s_axilite port = return bundle = control
	...
	for (int i=(0 + out_offset[0]); i<(out_shape[0] + out_end_offset[0]); i++){
		for (int j=(0 + out_offset[1]); j<(out_shape[1] + out_end_offset[1]); j++){
			out[out_lin_offset + i*out_stride[0] + j*out_stride[1]] = A[A_lin_offset + (i + A_offset[0])*A_stride[0] + (j + A_offset[1])*A_stride[1]] - B[B_lin_offset + (i + B_offset[0])*B_stride[0] + (j + B_offset[1])*B_stride[1]];
		}
	}
}