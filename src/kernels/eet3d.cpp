extern "C" void eet3d(double* A, double* B, int* A_stride, int* B_stride, int* A_offset, int* B_offset, int A_lin_offset, int B_lin_offset, double* out, int* out_shape, int* out_stride, int* out_offset, int* out_end_offset, int out_lin_offset) {
	...
	for (int i=(0 + out_offset[0]); i<(out_shape[0] + out_end_offset[0]); i++){
		for (int j=(0 + out_offset[1]); j<(out_shape[1] + out_end_offset[1]); j++){
			for (int k=(0 + out_offset[2]); k<(out_shape[2] + out_end_offset[2]); k++){
				...
				B_ind = (i + B_offset[0])*B_stride[0] + (j + B_offset[1])*B_stride[1] + (k + B_offset[2])*B_stride[2] + B_lin_offset;
				O_ind = i*out_stride[0] + j*out_stride[1] + k*out_stride[2] + out_lin_offset + out_lin_offset;
				A_val = A[A_ind];
				B_val = B[B_ind];
				if (A_val == B_val){
					out[O_ind] = 1;
