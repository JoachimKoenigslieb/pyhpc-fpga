extern "C" void mult4d(double* A, double* B, int* A_stride, int* B_stride, double* out, int* out_shape, int* out_stride) {
		...
		for (int j=0; j<out_shape[1]; j++){
			for (int k=0; k<out_shape[2]; k++){
				for (int l=0; l<out_shape[3]; l++){
					out[i *out_stride[0] + j*out_stride[1] + k*out_stride[2] + l*out_stride[3]] = A[i*A_stride[0] + j*A_stride[1] + k*A_stride[2] + l*A_stride[3]] * B[i*B_stride[0] + j*B_stride[1] + k*B_stride[2] + l*B_stride[3]];
				}
			}
		}
	}
